...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const [integrations, setIntegrations] = useState(
    currentUser.integrations || { googleCalendarUrl: '', smsApiKey: '', automationWebhook: '' }
  );
  ...
  const safeExportStringify = (data: any) => {
    ...
    try {
      return JSON.stringify(prune(data), null, 2);
    } catch (e) {
      return '{}';
    }
  };
  ...
  const handleExportData = () => {
    try {
      ...
      downloadAnchorNode.setAttribute(
        'download',
        `backup_belair_${new Date().toISOString().split('T')[0]}.json`
      );
  ...
  const handleClearAllCaches = async () => {
    if (
      window.confirm(
        'Vider tous les caches (Service Worker + localStorage + sessionStorage) ? Vous devrez vous reconnecter.'
      )
    ) {
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in p-2 md:p-0">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div
              className={`p-3 rounded-full ${theme === 'dark' ? 'bg-indigo-900 text-indigo-300' : 'bg-amber-100 text-amber-700'}`}
            >
              {theme === 'dark' ? <Moon size={24} /> : <Sun size={24} />}
            </div>
            <div>
              <h3 className="font-semibold text-slate-800 dark:text-slate-100 dark:text-white">
                Apparence
              </h3>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-4 py-4 md:px-6 flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
            Paramètres
          </h2>
        </div>
        ...
        <div className="p-4 md:p-6 min-h-[500px]">
          {activeTab === 'data' && (
            <div className="space-y-6">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
                    Gestion des Données
                  </h3>
            ...
            <div className="space-y-6">
              ...
              {buildInfo && (
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
                  <h4 className="font-semibold text-slate-800 dark:text-slate-100 dark:text-white mb-2">
                    Informations de Version
                  </h4>
                  <div className="space-y-1 text-sm text-slate-700 dark:text-slate-300">
                    <p>
                      <span className="font-medium">Version:</span> {buildInfo.version}
                    </p>
                    <p>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400">
                    Le Service Worker peut garder l'ancienne version en cache. Désactivez-le si les
              ...
              {/* Cache Management */}
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
                <h4 className="font-semibold text-slate-800 dark:text-slate-100 dark:text-white mb-3">
                  Gestion des Caches
                </h4>
                <div className="space-y-3">
                  <button
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm text-blue-800 dark:text-blue-200">
                  ...
                  <li>
                    Allez dans l'onglet <strong>"Application"</strong> ou <strong>"Storage"</strong>
                  </li>
                  <li>
                    Dans le menu de gauche, cliquez sur <strong>"Service Workers"</strong>
                  </li>
                  <li>
                    Cliquez sur <strong>"Unregister"</strong> pour désactiver le Service Worker
                  </li>
                  <li>
                    Allez dans <strong>"Clear storage"</strong> et cochez tout
                  </li>
                  <li>
                    Cliquez sur <strong>"Clear site data"</strong>
                  </li>
          ...
          {activeTab === 'users' && isAdmin && (
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
                    Utilisateurs
                  </h3>
                  <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white dark:text-white dark:text-white">
                    Gérez les comptes d'accès.
                  </p>
                </div>
                <button
              ...
              {isAddUserOpen && (
                <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-600 rounded-xl p-6 mb-6">
                  <h4 className="font-semibold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white mb-4">
                    Nouvel Utilisateur
                  </h4>
                  <form
                  ...
                  >
                    ...
                    <div className="md:col-span-2 flex justify-end space-x-3 mt-2">
                      <button
                        ...
                        className="px-4 py-2 text-slate-700 dark:text-slate-200 dark:text-white bg-slate-200 dark:bg-slate-600 rounded text-sm"
                      >
                        Annuler
                      </button>
                      <button
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <div className="truncate max-w-[120px] md:max-w-none">
                              <div className="font-medium text-slate-900 dark:text-white dark:text-white dark:text-white truncate">
                                {user.fullName}
                              </div>
                            </div>
                          </div>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${user.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}
                          >
                            {user.role}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-slate-700 dark:text-slate-200 dark:text-white hidden md:table-cell">
                          {user.email}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center">
                            ...
                            <button
                              onClick={() =>
                                setVisiblePasswordId(visiblePasswordId === user.id ? null : user.id)
                              }
                              ...
                              title={
                                visiblePasswordId === user.id ? 'Masquer' : 'Voir le mot de passe'
                              }
                            >
                        ...
                        <td className="px-4 py-3 text-right">
                          {user.id !== currentUser.id && (
                            <button
                              ...
                              className="p-1.5 text-slate-700 dark:text-slate-200 dark:text-white hover:text-red-500"
                            >
                              <Trash2 size={16} />
                            </button>
                          )}
          ...
          {activeTab === 'integrations' && isAdmin && (
            <div className="max-w-3xl space-y-8 animate-in fade-in">
              <div>
                ...
                <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white dark:text-white dark:text-white">
                  Connectez vos services tiers pour automatiser votre activité.
                </p>
              </div>
              ...
              <div className="bg-purple-50 dark:bg-purple-900/10 p-6 rounded-xl border-2 border-purple-200 dark:border-purple-800 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-start space-x-4 mb-4 relative z-10">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white text-lg">
                      Automatisation (Make.com)
                    </h4>
                    <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white mt-1">
                      Copiez votre lien Webhook ici pour activer l'envoi automatique vers Google
                      Agenda et SMS.
                    </p>
                  </div>
                </div>
                <div className="mt-6 relative z-10">
                  <label className="block text-xs font-bold text-purple-800 dark:text-purple-300 uppercase mb-2">
                    URL du Webhook
                  </label>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) =>
                        setIntegrations({ ...integrations, automationWebhook: e.target.value })
                      }
                      className={inputClass}
              ...
              <div className="bg-white dark:bg-slate-900 dark:bg-slate-800/30 p-6 rounded-xl border border-slate-200 dark:border-slate-800">
                <div className="flex items-start space-x-4 mb-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
                      Google Agenda (Visuel)
                    </h4>
                ...
                <div className="mt-4">
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-200 dark:text-white dark:text-white dark:text-white uppercase mb-2">
                    URL d'intégration (src iframe)
                  </label>
                  <input
                    type="text"
                    value={integrations.googleCalendarUrl}
                    onChange={(e) =>
                      setIntegrations({ ...integrations, googleCalendarUrl: e.target.value })
                    }
                    className={inputClass}
          ...
          {activeTab === 'users' && !isAdmin && (
            <div className="flex flex-col items-center justify-center p-12 text-center text-slate-700 dark:text-slate-200 dark:text-white">
              ...
              <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 dark:text-white">
                Accès Restreint
              </h3>
