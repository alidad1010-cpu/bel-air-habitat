...
import { Search, Plus, Trash2, Mail, Phone, MapPin, FileText, Upload, Handshake, X, ExternalLink, Briefcase, Sparkles, User, Filter, Check } from 'lucide-react';
...
const PartnersPage: React.FC<PartnersPageProps> = ({ clients, onAddClient, onUpdateClient, onDeleteClient }) => {
    ...
    const getDocStatus = (docs: ClientDocument[], type: string) => {
        ...
        if (!doc) return { status: 'MISSING', label: 'Manquant', color: 'text-red-500 bg-red-50 border-red-100' };
        ...
        if (doc.expiryDate) {
            ...
            if (diffDays < 0) return { status: 'EXPIRED', label: 'Expiré', color: 'text-slate-900 dark:text-white dark:text-white bg-red-500 font-bold border-red-600' };
            if (diffDays < 30) return { status: 'WARNING', label: `Exp.${diffDays} j`, color: 'text-orange-700 bg-orange-100 border-orange-200' };
        }

        return { status: 'OK', label: 'Valide', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' };
    };
    ...
    const handleAddSubmit = (e: React.FormEvent) => {
        ...
        setNewPartner({ name: '', email: '', phone: '', address: '', zipCode: '', city: '', type: 'SOUS_TRAITANT', siret: '', vatNumber: '' });
    };
        ...
        <div className="space-y-6 animate-fade-in relative">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white">Gestion des contrats, assurances et documents administratifs.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {partners.map((partner) => {
                    ...
                    return (
                        <div key={partner.id} onClick={() => setSelectedPartner(partner)} className={`bg-white dark:bg-slate-900 rounded-xl border shadow-sm hover:shadow-md transition-all cursor-pointer group p-5 relative overflow-hidden ${hasIssues ? 'border-red-300 ring-1 ring-red-300' : 'border-slate-200 dark:border-slate-800'} `}>
                            {hasIssues && <div className="absolute top-0 right-0 bg-red-500 text-slate-900 dark:text-white dark:text-white text-[9px] px-2 py-0.5 font-bold uppercase rounded-bl">Action Requise</div>}

                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center space-x-3">
                                    <div className={`h-12 w-12 rounded-full flex items-center justify-center font-bold text-lg ${partner.type === 'SOUS_TRAITANT' ? 'bg-cyan-100 text-cyan-700' : 'bg-amber-100 text-amber-700'} `}>
                                        {partner.name.charAt(0).toUpperCase()}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">{partner.name}</h3>
                            ...
                            <div className="space-y-2 mb-4 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg">
                                <div className="flex justify-between text-xs items-center">
                                    <span className="text-slate-700 dark:text-slate-200 dark:text-white font-medium">Kbis (3 mois)</span>
                                    <span className={`px-1.5 py-0.5 rounded border text-[10px] font-bold ${kbisStatus.color} `}>{kbisStatus.label}</span>
                                </div>
                                <div className="flex justify-between text-xs items-center">
                                    <span className="text-slate-700 dark:text-slate-200 dark:text-white font-medium">Décennale</span>
                                    <span className={`px-1.5 py-0.5 rounded border text-[10px] font-bold ${insuranceStatus.color} `}>{insuranceStatus.label}</span>
                                </div>
                                <div className="flex justify-between text-xs items-center">
                                    <span className="text-slate-700 dark:text-slate-200 dark:text-white font-medium">URSSAF (6 mois)</span>
                                    <span className={`px-1.5 py-0.5 rounded border text-[10px] font-bold ${urssafStatus.color} `}>{urssafStatus.label}</span>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
                        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">Nouveau Partenaire</h3>
                            <button onClick={() => setIsModalOpen(false)}><X className="text-slate-700 dark:text-slate-200 dark:text-white" /></button>
                        </div>
                        <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
                            <div>
                                <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200">Type</label>
                                <select
                                    value={newPartner.type}
                                    onChange={e => setNewPartner({ ...newPartner, type: e.target.value as ClientType })}
                            ...
                            <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200">Raison Sociale</label><input required className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white text-sm" value={newPartner.name} onChange={e => setNewPartner({ ...newPartner, name: e.target.value })} /></div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200">SIRET</label><input className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white text-sm" value={newPartner.siret} onChange={e => setNewPartner({ ...newPartner, siret: e.target.value })} /></div>
                                <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200">TVA Intra.</label><input className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white text-sm" value={newPartner.vatNumber} onChange={e => setNewPartner({ ...newPartner, vatNumber: e.target.value })} /></div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200">Email</label><input type="email" className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white text-sm" value={newPartner.email} onChange={e => setNewPartner({ ...newPartner, email: e.target.value })} /></div>
                                <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200">Téléphone</label><input className="w-full p-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white text-sm" value={newPartner.phone} onChange={e => setNewPartner({ ...newPartner, phone: e.target.value })} /></div>
                            </div>
                            <button className="w-full bg-emerald-600 text-white py-2 rounded-lg font-bold">Enregistrer</button>
                        </form>
                    </div>
                </div>
            )}
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden shadow-2xl flex flex-col">
                        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
                            <div className="flex items-center">
                                <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-lg mr-3 ${selectedPartner.type === 'SOUS_TRAITANT' ? 'bg-cyan-600 text-slate-900 dark:text-white' : 'bg-amber-600 text-slate-900 dark:text-white'} `}>
                                    {selectedPartner.name.charAt(0).toUpperCase()}
                                </div>
                                <div>
                                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">{selectedPartner.name}</h2>
                                    <p className="text-xs text-slate-700 dark:text-slate-200 dark:text-white">SIRET: {selectedPartner.siret || 'N/A'} • TVA: {selectedPartner.vatNumber || 'N/A'}</p>
                                </div>
                            </div>
                            <div className="flex items-center space-x-2">
                                <button onClick={() => { if (confirm('Supprimer ?')) { onDeleteClient(selectedPartner); setSelectedPartner(null); } }} className="text-slate-700 dark:text-slate-200 dark:text-white hover:text-red-500 p-2"><Trash2 size={20} /></button>
                                <button onClick={() => setSelectedPartner(null)} className="text-slate-700 dark:text-slate-200 dark:text-white hover:text-slate-700 dark:text-slate-200 p-2"><X size={24} /></button>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                ...
                                <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <h3 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white mb-4 flex items-center border-b border-slate-100 pb-2">
                                        <FileText size={18} className="mr-2 text-emerald-500" /> Documents Légaux & Conformité
                                    </h3>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {[
                                            ...
                                            return (
                                                <div key={docType.id} className="flex flex-col p-3 border rounded-lg bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 relative hover:border-emerald-300 transition-colors">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="font-bold text-sm text-slate-700 dark:text-slate-200 dark:text-white">{docType.label}</div>
                                                        <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold uppercase border ${status.color} `}>
                                                            {status.label}
                                                        </span>
                                                    </div>
                                                    ...
                                                    {doc ? (
                                                        <div className="flex justify-between items-end mt-auto">
                                                            ...
                                                            <a href={doc.url} target="_blank" className="p-1.5 bg-white dark:bg-slate-900 border rounded text-emerald-600 hover:text-emerald-800"><ExternalLink size={14} /></a>
                                                        </div>
                                                    ) : (
                                                        <div className="mt-auto pt-2">
                                                            ...
                                                            <div className="absolute inset-0 z-10 opacity-0 cursor-pointer" onClick={() => {
                                ...
                                <div className="space-y-6">
                                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <p className="text-[10px] text-slate-700 dark:text-slate-200 dark:text-white text-center leading-tight">
                                                Génère un contrat PDF basé sur le modèle juridique 2022 (Art. 283-2 CGI Autoliquidation).
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                        <h3 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white mb-4 flex items-center border-b border-slate-100 pb-2">
                                            <User size={18} className="mr-2 text-slate-700 dark:text-slate-200 dark:text-white" /> Coordonnées
                                        </h3>
                                        <div className="space-y-3 text-sm">
                                            <div>
                                                <label className="text-xs font-bold text-slate-700 dark:text-slate-200 dark:text-white uppercase">Adresse</label>
                                                <p className="font-medium">{selectedPartner.address}<br />{selectedPartner.zipCode} {selectedPartner.city}</p>
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-slate-700 dark:text-slate-200 dark:text-white uppercase">Email</label>
                                                <a href={`mailto:${selectedPartner.email} `} className="block text-emerald-600 truncate">{selectedPartner.email}</a>
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-slate-700 dark:text-slate-200 dark:text-white uppercase">Téléphone</label>
                                                <a href={`tel:${selectedPartner.phone} `} className="block font-bold">{selectedPartner.phone}</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
