...
import { Search, Plus, Trash2, Mail, Phone, MapPin, User, FileText, Upload, Briefcase, Handshake, CheckCircle, AlertTriangle, Clock, ExternalLink, Loader2, Save, X, Sparkles } from 'lucide-react';
import { Client, Project, ClientType, ClientDocument } from '../types';
...
const PartnersPage: React.FC<PartnersPageProps> = ({ clients, onAddClient, onUpdateClient, onDeleteClient }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: string) => {
      ...
      setIsUploading(true);
      try {
          const path = `partners/${selectedPartner.id}/documents/${Date.now()}_${file.name}`;
          let url;
          try {
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
                    ...
                    <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
                        <div>
                             <label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white">Type</label>
                             <select 
                        ...
                        <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white">Raison Sociale</label><input required className="w-full p-2 border rounded-lg dark:bg-slate-800" value={newPartner.name} onChange={e => setNewPartner({...newPartner, name: e.target.value})}/></div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white">SIRET</label><input className="w-full p-2 border rounded-lg dark:bg-slate-800" value={newPartner.siret} onChange={e => setNewPartner({...newPartner, siret: e.target.value})}/></div>
                            <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white">TVA Intra.</label><input className="w-full p-2 border rounded-lg dark:bg-slate-800" value={newPartner.vatNumber} onChange={e => setNewPartner({...newPartner, vatNumber: e.target.value})}/></div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white">Email</label><input type="email" className="w-full p-2 border rounded-lg dark:bg-slate-800" value={newPartner.email} onChange={e => setNewPartner({...newPartner, email: e.target.value})}/></div>
                            <div><label className="text-xs font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white">Téléphone</label><input className="w-full p-2 border rounded-lg dark:bg-slate-800" value={newPartner.phone} onChange={e => setNewPartner({...newPartner, phone: e.target.value})}/></div>
                        </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden shadow-2xl flex flex-col">
                    <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
                        <div className="flex items-center">
                             <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-lg mr-3 ${selectedPartner.type === 'SOUS_TRAITANT' ? 'bg-cyan-600 text-slate-900 dark:text-white dark:text-white' : 'bg-amber-600 text-slate-900 dark:text-white dark:text-white'}`}>
