  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <span className="text-[10px] font-mono font-bold text-slate-700 dark:text-slate-200">Code Affaire: {project.businessCode || '---'}</span>
      ...
      <p className="text-xs text-slate-700 dark:text-slate-200 mb-3 line-clamp-2">{project.description}</p>
      
      {/* TAGS */}
      {project.tags && project.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
              {project.tags.slice(0, 3).map(tag => (
                  <span key={tag} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-medium">
                      <Tag size={8} className="mr-1 opacity-50"/> {tag}
                  </span>
              ))}
              {project.tags.length > 3 && <span className="text-[10px] text-slate-700 dark:text-slate-200">+{project.tags.length - 3}</span>}
          </div>
      )}

      <div className="space-y-1 mb-3">
        <div className="flex items-center text-xs text-slate-700 dark:text-slate-200">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
        <div className="flex items-center space-x-2">
            ...
            <span className="text-xs font-medium text-slate-700 dark:text-slate-200 truncate max-w-[80px]">{project.client.name}</span>
        </div>
