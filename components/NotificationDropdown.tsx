  ...
  return (
    <div className="absolute right-0 top-12 w-80 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
      <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
          ...
          <button onClick={onClose} className="text-slate-700 dark:text-slate-200 hover:text-slate-700 dark:text-slate-200">
              <X size={16} />
          </button>
      </div>
      ...
      <div className="max-h-[300px] overflow-y-auto">
          {notifications.length === 0 ? (
              <div className="p-8 text-center text-slate-700 dark:text-slate-200 flex flex-col items-center">
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
                  {notifications.map(notif => (
                      ...
                      >
                          ...
                          <div>
                              <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{notif.title}</p>
                              <p className="text-xs text-slate-700 dark:text-slate-200 mt-0.5 line-clamp-2">{notif.message}</p>
                              <span className="text-[10px] text-slate-700 dark:text-slate-200 mt-1 block">
