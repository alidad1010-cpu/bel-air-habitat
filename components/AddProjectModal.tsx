...
import { X, Mail, Phone, Calendar, Hash, User, MapPin, Briefcase, Sparkles, Wand2, Loader2, ChevronDown, ChevronUp, Building } from 'lucide-react';
...
const AddProjectModal: React.FC<AddProjectModalProps> = ({ isOpen, onClose, onAdd, initialClient, clients = [], projects = [] }) => {
    // Autocomplete State
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const tags = formData.tagInput
            ? formData.tagInput.split(',').map(t => t.trim()).filter(t => t.length > 0)
            : [];
    ...
    const inputClass = "w-full p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-base md:text-sm text-slate-900 dark:text-white placeholder-slate-400";
    const labelClass = "block text-xs font-bold text-slate-800 dark:text-slate-200 uppercase mb-1 ml-1";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col animate-in fade-in zoom-in-95">
                ...
                <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center sticky top-0 bg-white dark:bg-slate-900 z-20">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">Nouveau Dossier</h2>
                        <div className="flex items-center space-x-2 text-sm mt-1">
                            <span className="text-slate-700 dark:text-slate-200 dark:text-white dark:text-white dark:text-white">Référence:</span>
                    ...
                    <button onClick={onClose} className="text-slate-700 dark:text-slate-200 dark:text-white hover:text-slate-700 dark:text-slate-200 dark:hover:text-slate-200 p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors">
                        <X size={24} />
                    </button>
                </div>
                ...
                <div className="px-6 pt-6 pb-2">
                    ...
                    >
                        <div className="flex items-center font-bold">
                            <Sparkles size={20} className={`mr-2 ${showMagicInput ? 'text-indigo-600' : 'text-slate-900 dark:text-white dark:text-white'}`} />
                            <span className={showMagicInput ? 'text-indigo-900 dark:text-indigo-200' : 'text-slate-900 dark:text-white dark:text-white'}>
                                Remplissage Magique IA
                            </span>
                        </div>
                        {showMagicInput ? <ChevronUp size={20} className="text-indigo-400" /> : <ChevronDown size={20} className="text-slate-900 dark:text-white dark:text-white" />}
                    </button>
                    ...
                    {showMagicInput && (
                        <div className="mt-3 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800 animate-in slide-in-from-top-2">
                            <p className="text-xs text-indigo-700 dark:text-indigo-300 mb-2 font-medium">
                                Collez ici un email, un SMS ou vos notes en vrac. L'IA va remplir le formulaire pour vous.
                            </p>
                            <textarea
                                value={magicText}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-8 pt-2">
                    ...
                    <div className="bg-slate-50/80 dark:bg-slate-800/30 p-5 rounded-xl border border-slate-200/60 dark:border-slate-600/50">
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white flex items-center mb-4 text-sm">
                            <div className="bg-indigo-100 dark:bg-indigo-900/50 p-1.5 rounded-lg mr-2"><User size={16} className="text-indigo-600 dark:text-indigo-400" /></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        {filteredClients.map((client, idx) => (
                                            ...
                                            >
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    {client.type && <span className="text-[9px] bg-slate-100 dark:bg-slate-500 px-1 rounded uppercase">{client.type}</span>}
                                                </div>
                                                <span className="text-xs text-slate-700 dark:text-slate-200 dark:text-white dark:text-white dark:text-white">{client.email} • {client.city}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Mail size={16} className="absolute left-3 top-3 text-slate-700 dark:text-slate-200 dark:text-white" />
                                    <input type="email" value={formData.clientEmail} onChange={e => setFormData({ ...formData, clientEmail: e.target.value })} className={`${inputClass} pl-9`} placeholder="client@email.com" />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Phone size={16} className="absolute left-3 top-3 text-slate-700 dark:text-slate-200 dark:text-white" />
                                    <input type="text" value={formData.clientPhone} onChange={e => setFormData({ ...formData, clientPhone: e.target.value })} className={`${inputClass} pl-9`} placeholder="06..." />
                                </div>
                            </div>
                            ...
                            <div className="col-span-1 md:col-span-2">
                                ...
                                />
                                <p className="text-[10px] text-slate-700 dark:text-slate-200 dark:text-white mt-1 ml-1">Adresse utilisée pour la facturation.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white flex items-center mb-4 text-sm px-1">
                            <div className="bg-emerald-100 dark:bg-emerald-900/50 p-1.5 rounded-lg mr-2"><Briefcase size={16} className="text-emerald-600 dark:text-emerald-400" /></div>
                            CHANTIER & DÉTAILS
                        </h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                            <div className="col-span-1 md:col-span-2">
                                ...
                                <input type="text" required value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className={`${inputClass} font-bold`} placeholder="Ex: Rénovation Salle de Bain" />
                            </div>
                            ...
                            <div className="col-span-1 md:col-span-2">
                                ...
                                <AddressAutocomplete
                                    ...
                                    onSelect={(result) => setFormData(prev => ({ ...prev, siteAddress: result.fullAddress, siteLat: result.lat, siteLng: result.lng }))}
                                ...
                                />
                                <p className="text-[10px] text-emerald-600/70 mt-1 ml-1 font-medium">Laissez vide si identique à l'adresse de facturation.</p>
                            </div>

                            <div>
                                <label className={labelClass}>Type de Dossier</label>
                                <select value={formData.folderType} onChange={e => setFormData({ ...formData, folderType: e.target.value })} className={inputClass}>
                            ...
                            <div>
                                ...
                                <input type="number" value={formData.budget} onChange={e => setFormData({ ...formData, budget: e.target.value })} className={inputClass} placeholder="0.00" />
                            </div>
                            ...
                            <div className="md:col-span-2 grid grid-cols-2 gap-4">
                                ...
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-1 rounded-lg border border-blue-100 dark:border-blue-800 relative">
                                    <label className="absolute -top-2 left-2 px-1 bg-white dark:bg-slate-900 text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase rounded">Début Envisagé</label>
                                    <div className="relative">
                                        <Calendar size={16} className="absolute left-3 top-3 text-blue-600 dark:text-blue-400" />
                                        <input
                                            type="date"
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900/20 p-1 rounded-lg border border-slate-200 dark:border-slate-800 relative">
                                    <label className="absolute -top-2 left-2 px-1 bg-white dark:bg-slate-900 text-[10px] font-bold text-slate-700 dark:text-slate-200 dark:text-white uppercase rounded">TVA & Régime</label>
                                    <select
                    ...
                    <div className="flex justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                        <button type="button" onClick={onClose} className="text-slate-700 dark:text-slate-200 dark:text-white hover:text-slate-700 dark:text-slate-200 dark:text-white dark:text-white dark:hover:text-slate-200 font-medium px-6 py-3 mr-2">
                            Annuler
                        </button>
                        <button type="submit" className="bg-slate-900 dark:bg-slate-800 hover:bg-emerald-600 dark:hover:bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-slate-200 dark:shadow-none transition-all hover:scale-105 flex items-center">
                            Créer le Dossier
                        </button>
                    </div>
                </form>
