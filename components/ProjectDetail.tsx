...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onBack, onSave, onDelete, onDuplicate, onTriggerAutomation, clients = [] }) => {
    ...
    const handleAddAppointment = () => {
        ...
        setNewAppointment({ date: new Date().toISOString().split('T')[0], startTime: '09:00', endTime: '10:00', title: 'RDV Chantier' });
    };
    ...
    const handleBack = () => {
        if (saveStatus === 'modified') {
            if (window.confirm("Vous avez des modifications non enregistrées. Voulez-vous les enregistrer avant de quitter ?")) {
                onSave(formData);
            }
        }
        onBack();
    ...
    const handleDelete = () => {
        ...
        if (window.confirm("⚠️ Êtes-vous sûr de vouloir SUPPRIMER DÉFINITIVEMENT ce dossier ?\n\nCette action est irréversible.")) {
    ...
    const generateBDC = () => {
        ...
        const tasksLabels = formData.tasks?.filter(t => selectedTasks.includes(t.id)).map(t => t.label) || [];
        ...
        setNewBDC({ subName: '', subId: '', amount: '', selectedTasks: [], desc: '', startDate: '', endDate: '' });
    };
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                if (originalFile.size > MAX_PHOTO_INPUT_SIZE_MB * 1024 * 1024) {
                    if (!window.confirm(`La photo "${originalFile.name}" est très lourde (${(originalFile.size / 1024 / 1024).toFixed(1)}MB). Cela risque de bloquer l'application. Continuer ?`)) {
                        continue;
                    }
                }
    ...
    const isLate = formData.endDate && new Date(formData.endDate) < new Date() &&
    ...
    const isFailedState = formData.status === ProjectStatus.LOST || formData.status === ProjectStatus.REFUSED || formData.status === ProjectStatus.CANCELLED;
    ...
    const renderLifecycleActions = () => {
        ...
        if (s === ProjectStatus.NEW) {
            return (
                <button onClick={() => handleStatusChange(ProjectStatus.QUOTE_SENT)} className="bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm flex items-center print:hidden">
        ...
        if (s === ProjectStatus.QUOTE_SENT) {
            return (
                <div className="flex space-x-2 print:hidden">
                    <button onClick={() => handleStatusChange(ProjectStatus.LOST)} className="bg-rose-100 text-rose-700 hover:bg-rose-200 px-4 py-2 rounded-lg text-sm font-bold">
                        Client Refuse
                    </button>
                    <button onClick={() => handleStatusChange(ProjectStatus.VALIDATED)} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm flex items-center">
        ...
        if (s === ProjectStatus.VALIDATED) {
            return (
                <button onClick={() => handleStatusChange(ProjectStatus.IN_PROGRESS)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm flex items-center animate-pulse print:hidden">
        ...
        if (s === ProjectStatus.IN_PROGRESS) {
            return (
                <button onClick={() => handleStatusChange(ProjectStatus.WAITING_VALIDATION)} className="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm flex items-center print:hidden">
        ...
        if (s === ProjectStatus.WAITING_VALIDATION) {
            ...
            return (
                <div className="flex items-center space-x-2 print:hidden">
                    <button onClick={() => handleStatusChange(ProjectStatus.REFUSED)} className="bg-red-100 text-red-700 hover:bg-red-200 px-3 py-2 rounded-lg text-xs font-bold">
                        Rejeté
                    </button>

                    {canComplete ? (
                        <button onClick={() => handleStatusChange(ProjectStatus.COMPLETED)} className="bg-slate-800 hover:bg-black text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm flex items-center">
                        ...
                        <div className="flex flex-col items-end">
                            <div className="flex items-center text-xs text-orange-600 font-bold bg-orange-50 px-3 py-1.5 rounded-lg border border-orange-200">
                                ...
                                Manque: {!hasPV && <span className="mx-1 underline">PV</span>} {!hasInvoice && <span className="mx-1 underline">Facture</span>}
                            </div>
                        </div>
                    )}
                </div>
        ...
        if (isFailedState) {
            return <span className="px-3 py-1 bg-slate-100 text-slate-700 dark:text-slate-200 dark:text-white rounded text-xs font-bold uppercase print:hidden">Dossier Clos ({s})</span>
        }
    ...
    return (
        <div className="bg-white dark:bg-slate-900 min-h-screen flex flex-col relative duration-300">
            ...
            {/* PRINT VIEW OVERLAY */}
            {showPrintView && (
                <div className="fixed inset-0 z-[9999] bg-white dark:bg-slate-900 text-black dark:text-white p-8 overflow-y-auto animate-in fade-in">
                    {/* FIXED CLOSE BUTTON FOR MOBILE/DESKTOP */}
                    <div className="fixed top-4 right-4 z-[10000] flex gap-2 print:hidden">
                        <button onClick={() => window.print()} className="bg-slate-900 text-white p-3 rounded-full shadow-lg hover:bg-emerald-600 transition-colors" title="Lancer l'impression"><Printer /></button>
                        <button onClick={() => setShowPrintView(false)} className="bg-white dark:bg-slate-900 border-2 border-slate-200 text-slate-700 dark:text-slate-200 dark:text-white p-3 rounded-full shadow-lg hover:text-red-500 transition-colors" title="Fermer"><X /></button>
                    </div>

                    <div className="max-w-4xl mx-auto border-2 border-slate-900 p-8 min-h-[1000px] relative">
                        ...
                        <div className="flex justify-between items-start border-b-4 border-slate-900 pb-4 mb-6">
                            <div>
                                ...
                                <p className="text-slate-700 dark:text-slate-200 dark:text-white font-bold mt-1">Dossier: {formData.businessCode}</p>
                            </div>
                            <div className="text-right">
                                <h2 className="text-xl font-bold">BEL AIR HABITAT</h2>
                                <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white">19 B Rue de la Tourelle, 95170 Deuil-la-Barre</p>
                        ...
                        <div className="grid grid-cols-2 gap-6 mb-8">
                            <div className="border border-slate-300 p-4 rounded-lg bg-slate-50">
                                <h3 className="font-bold text-slate-900 dark:text-white uppercase text-sm mb-3 flex items-center"><User size={16} className="mr-2" /> Client & Contact</h3>
                                <p className="font-bold text-lg">{formData.client?.name || "Nom inconnu"}</p>
                                <p className="mt-1">{formData.client?.email || "Email inconnu"}</p>
                                <p className="font-mono font-bold mt-1">{formData.client?.phone || "Tél inconnu"}</p>
                            </div>
                            <div className="border border-slate-300 p-4 rounded-lg bg-slate-50">
                                <h3 className="font-bold text-slate-900 dark:text-white uppercase text-sm mb-3 flex items-center"><MapPin size={16} className="mr-2" /> Adresse Chantier</h3>
                                {/* Use siteAddress if available (construction site), else fall back to client address */}
                                <p className="text-lg font-medium">{formData.siteAddress || formData.client?.address || "Adresse inconnue"}</p>
                                {/* Only show City/Zip if it's client address fallback, or if we parse it from siteAddress (simple display here) */}
                                {!formData.siteAddress && <p>{formData.client?.zipCode} {formData.client?.city}</p>}
                            </div>
                        </div>

                        {/* Access & Logistics - CRITICAL FOR WORKERS */}
                        <div className="mb-8">
                            <h3 className="font-bold text-slate-900 dark:text-white uppercase text-sm mb-2 border-b border-slate-200 pb-1">Accès & Clés</h3>
                            <div className="grid grid-cols-3 gap-4">
                                <div className="border p-3 rounded bg-white dark:bg-slate-900">
                                    <span className="text-xs text-slate-700 dark:text-slate-200 dark:text-white uppercase font-bold block">Digicode</span>
                                    <span className="font-mono text-xl font-bold">{formData.accessInfo?.digicode || "---"}</span>
                                </div>
                                <div className="border p-3 rounded bg-white dark:bg-slate-900">
                                    <span className="text-xs text-slate-700 dark:text-slate-200 dark:text-white uppercase font-bold block">Étage / Porte</span>
                                    <span className="font-mono text-lg font-bold">{formData.accessInfo?.floor || "---"}</span>
                                </div>
                                <div className="border p-3 rounded bg-white dark:bg-slate-900">
                                    <span className="text-xs text-slate-700 dark:text-slate-200 dark:text-white uppercase font-bold block">Boîte à Clé</span>
                                    <span className="font-mono text-lg font-bold">{formData.accessInfo?.keysLocation || "---"}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Description */}
                        <div className="mb-8">
                            <h3 className="font-bold text-slate-900 dark:text-white uppercase text-sm mb-2 border-b border-slate-200 pb-1">Description / Cahier des Charges</h3>
                            <p className="text-sm whitespace-pre-wrap leading-relaxed">{formData.description}</p>
                        </div>

                        {/* Tasks Checklist */}
                        <div className="mb-8">
                            <h3 className="font-bold text-slate-900 dark:text-white uppercase text-sm mb-2 border-b border-slate-200 pb-1">Détail des Prestations à Réaliser (Sans Prix)</h3>
                            {(!formData.tasks || formData.tasks.length === 0) ? (
                                <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white italic">Aucune prestation spécifique listée.</p>
                            ) : (
                                <div className="space-y-2">
                                    {formData.tasks.map(t => (
                                        <div key={t.id} className="flex items-center">
                                            <div className={`w-5 h-5 border-2 border-slate-400 mr-3 rounded flex items-center justify-center ${t.isDone ? 'bg-slate-200' : 'bg-white dark:bg-slate-900'}`}>
                                                {t.isDone && <Check size={14} />}
                                            </div>
                                            <span className={t.isDone ? 'line-through text-slate-700 dark:text-slate-200 dark:text-white' : 'font-medium'}>{t.label}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="border-2 border-dashed border-slate-300 rounded-lg h-40 p-4 mt-auto">
                            <span className="text-slate-700 dark:text-slate-200 dark:text-white font-bold uppercase text-xs">Notes Chantier / Croquis</span>
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-lg w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl overflow-hidden relative">
                        <div className="h-12 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 px-4 shrink-0">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <button onClick={() => setPreviewDoc(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors text-slate-700 dark:text-slate-200 dark:text-white">
                                    <X size={20} />
                                </button>
                            </div>
                        </div>
            ...
            <div className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm print:hidden">
                <div className="px-4 py-3 flex justify-between items-center">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white flex items-center text-lg leading-tight">
                                {formData.title}
                                {saveStatus === 'modified' && <span className="ml-2 w-2 h-2 rounded-full bg-amber-500 animate-pulse" title="Modifié"></span>}
                                {/* Weather Widget */}

                            </h2>
                            <div className="flex items-center space-x-2 mt-0.5">
                                <span className="text-xs text-slate-700 dark:text-slate-200 dark:text-white font-mono">#{formData.businessCode || formData.id}</span>
                                {isLate && (
                                ...
                                <button
                                    ...
                                    className={`flex items-center text-[10px] font-bold px-2 py-0.5 rounded-full border transition-colors ml-2 ${formData.needsCallback
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <button onClick={handleSave} className={`flex items-center px-4 py-2 rounded-lg font-bold text-sm transition-all ${saveStatus === 'saved' ? 'bg-slate-100 text-slate-700 dark:text-slate-200 dark:bg-slate-800 dark:text-white' : 'bg-emerald-600 text-white shadow-lg shadow-emerald-200'}`}>
                            {saveStatus === 'saving' ? <Loader2 size={16} className="animate-spin mr-2" /> : <Save size={16} className="mr-2" />}
                            {saveStatus === 'saved' ? 'Enregistré' : 'Enregistrer'}
                        </button>

                        {/* DANGER ZONE: Explicit delete available here too for convenience */}
                        <button onClick={handleDelete} className="p-2 text-slate-700 dark:text-slate-200 dark:text-white hover:text-red-500 rounded-full hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors">
                            <Trash2 size={18} />
                        </button>
                    </div>
                </div>
                            ...
                            {steps.map((step, idx) => {
                                ...
                                return (
                                    <div key={step.status} className="flex items-center">
                                        <div className={`flex items-center px-3 py-1 rounded-full text-xs font-bold border transition-colors ${isCurrent ? 'bg-emerald-600 text-white border-emerald-600 shadow-md' :
                                            isCompleted ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800' :
                                        ...
                                        {idx < steps.length - 1 && (
                                            <div className={`h-0.5 w-4 mx-1 ${idx < currentStepIndex ? 'bg-emerald-300' : 'bg-slate-200 dark:bg-slate-800'}`}></div>
                                        )}
                                    </div>
                                );
                            })}
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-[#F3F4F6] dark:bg-slate-900 print:hidden">
                <div className="max-w-6xl mx-auto bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 min-h-[600px] p-6">
                    ...
                    {activeTab === 'Principal' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <h3 className={sectionTitleClass}><FileText size={18} className="mr-2" /> Informations Dossier</h3>
                                <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" readOnly value={formData.businessCode || ''} className={`${inputClass} bg-slate-100 dark:bg-slate-600 cursor-not-allowed`} />
                                    </div>
                                    <div>
                                        <label className={labelClass}>Type Dossier</label>
                                        <select value={formData.folderType || 'Particulier'} onChange={e => updateField('folderType', e.target.value)} className={inputClass}>
                                ...
                                <div className="pt-2">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-2">
                                        {formData.skills?.map(skill => (
                                            <span key={skill} className="inline-flex items-center px-2 py-1 rounded bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-bold border border-indigo-200 dark:border-indigo-700">
                                                <Hammer size={10} className="mr-1" /> {skill}
                                                <button onClick={() => handleRemoveSkill(skill)} className="ml-2 hover:text-red-500"><X size={10} /></button>
                                            </span>
                                        ))}
                                    </div>
                                    <form onSubmit={handleAddSkill} className="flex">
                                        ...
                                        />
                                        <button type="submit" className="bg-slate-100 hover:bg-slate-200 px-3 border border-l-0 border-slate-200 rounded-r-lg text-xs font-bold text-slate-700 dark:text-slate-200">
                                            <Plus size={14} />
                                        </button>
                                    </form>
                                </div>
                                ...
                                <div className="pt-2">
                                    ...
                                    />
                                    {formData.lat && <p className="text-[9px] text-slate-700 dark:text-slate-200 dark:text-white mt-1 flex items-center"><MapPin size={9} className="mr-1" /> GPS: {formData.lat}, {formData.lng}</p>}
                                </div>
                                ...
                                <div className="pt-2">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-2">
                                        {formData.tags?.map(tag => (
                                            <span key={tag} className="inline-flex items-center px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 dark:text-white text-xs font-bold border border-slate-200 dark:border-slate-600">
                                                <Tag size={10} className="mr-1" /> {tag}
                                                <button onClick={() => handleRemoveTag(tag)} className="ml-2 hover:text-red-500"><X size={10} /></button>
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="pt-2">
                                    <div className="flex justify-between items-center mt-4 mb-2">
                                        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white flex items-center"><Briefcase size={16} className="mr-2 text-indigo-500" /> Rendez-vous</h3>
                                        <button
                                        ...
                                        >
                                            {isAddingAppointment ? <X size={12} className="mr-1" /> : <Plus size={12} className="mr-1" />}
                                    ...
                                    <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 space-y-3">
                                        ...
                                        {isAddingAppointment && (
                                            <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-indigo-200 dark:border-indigo-700 mb-3 shadow-sm animate-in zoom-in-95">
                                                <h4 className="text-xs font-bold text-indigo-700 mb-2 uppercase">Ajouter un RDV</h4>
                                                <div className="space-y-2">
                                                    <input type="text" value={newAppointment.title} onChange={e => setNewAppointment({ ...newAppointment, title: e.target.value })} className={inputClass} placeholder="Titre (ex: Visite Technique)" />
                                                    <input type="date" value={newAppointment.date} onChange={e => setNewAppointment({ ...newAppointment, date: e.target.value })} className={inputClass} />
                                                    <div className="flex space-x-2">
                                                        <input type="time" value={newAppointment.startTime} onChange={e => setNewAppointment({ ...newAppointment, startTime: e.target.value })} className={inputClass} />
                                                        <input type="time" value={newAppointment.endTime} onChange={e => setNewAppointment({ ...newAppointment, endTime: e.target.value })} className={inputClass} />
                                                    </div>
                                                    <textarea value={newAppointment.note} onChange={e => setNewAppointment({ ...newAppointment, note: e.target.value })} className={inputClass} placeholder="Notes..." rows={2} />
                                                    <button onClick={handleAddAppointment} className="w-full bg-indigo-600 text-white py-2 rounded-lg text-xs font-bold hover:bg-indigo-700">Enregistrer le RDV</button>
                                                </div>
                                            </div>
                                        )}

                                        {(!formData.appointments || formData.appointments.length === 0) && !isAddingAppointment ? (
                                            <p className="text-xs text-center text-slate-700 dark:text-slate-200 dark:text-white py-2">Aucun rendez-vous programmé.</p>
                                        ) : (
                                            <div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                                                {formData.appointments?.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(apt => (
                                                    <div key={apt.id} className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-indigo-100 dark:border-indigo-900/50 flex justify-between group">
                                                        <div>
                                                            <div className="font-bold text-sm text-slate-800 dark:text-slate-100 dark:text-white dark:text-white flex items-center">
                                                                {apt.title}
                                                                {new Date(apt.date) < new Date() && <span className="ml-2 text-[10px] bg-slate-100 text-slate-700 dark:text-slate-200 dark:text-white px-1 rounded">Passé</span>}
                                                            </div>
                                                            <div className="text-xs text-slate-700 dark:text-slate-200 dark:text-white flex items-center mt-1">
                                                                <Calendar size={12} className="mr-1" /> {new Date(apt.date).toLocaleDateString()}
                                                                <Clock size={12} className="ml-2 mr-1" /> {apt.startTime}
                                                            </div>
                                                            {apt.note && <div className="text-xs text-slate-700 dark:text-slate-200 dark:text-white mt-1 italic line-clamp-1">{apt.note}</div>}
                                                        </div>
                                                        <button onClick={() => deleteAppointment(apt.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            <Trash2 size={14} />
                                                        </button>
                                                    </div>
                                                ))}
                                ...
                                <div className="pt-4 border-t border-slate-100 dark:border-slate-800 mt-4">
                                    <h3 className={sectionTitleClass}><Key size={18} className="mr-2 text-indigo-500" /> Logistique & Accès</h3>
                                    <div className="grid grid-cols-2 gap-4 bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800">
                                        <div>
                                            ...
                                            <input type="text" value={formData.accessInfo?.digicode || ''} onChange={e => updateAccess('digicode', e.target.value)} className={inputClass} placeholder="Ex: 1234A" />
                                        </div>
                                        <div>
                                            <label className={labelClass}>Étage / Porte</label>
                                            <input type="text" value={formData.accessInfo?.floor || ''} onChange={e => updateAccess('floor', e.target.value)} className={inputClass} placeholder="Ex: 3ème, Porte Droite" />
                                        </div>
                                        <div>
                                            <label className={labelClass}>Boîte à Clés</label>
                                            <div className="relative">
                                                <Key size={14} className="absolute left-3 top-3 text-slate-700 dark:text-slate-200 dark:text-white" />
                                                <input type="text" value={formData.accessInfo?.keysLocation || ''} onChange={e => updateAccess('keysLocation', e.target.value)} className={`${inputClass} pl-9`} placeholder="Code ou emplacement" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {formData.status === ProjectStatus.NEW && (
                                    <div className="pt-4 p-4 bg-sky-50 dark:bg-sky-900/10 rounded-xl border border-sky-100 dark:border-sky-800">
                                        ...
                                        />
                                        {uploadStatusMsg && <p className="text-xs text-sky-600 font-bold mt-2 animate-pulse">{uploadStatusMsg}</p>}
                                        <p className="text-[10px] text-sky-700/60 mt-1">Le statut passera automatiquement à "Devis Envoyé" et le montant HT sera extrait.</p>
                                    </div>
                                )}
                            </div>

                            <div className="space-y-4">
                                <h3 className={sectionTitleClass}><Calendar size={18} className="mr-2" /> Planification & Budget</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className={labelClass}>Date Début</label><input type="date" value={formData.startDate || ''} onChange={e => updateField('startDate', e.target.value)} className={inputClass} /></div>
                                    <div><label className={labelClass}>Date Fin (Prévue)</label><input type="date" value={formData.endDate || ''} onChange={e => updateField('endDate', e.target.value)} className={inputClass} /></div>
                                </div>
                                <div className="pt-2">
                                    <label className={labelClass}>Budget HT</label>
                                    <div className="relative">
                                        <Euro size={16} className="absolute left-3 top-2.5 text-slate-700 dark:text-slate-200 dark:text-white" />
                                        <input type="number" value={formData.budget || ''} onChange={e => updateField('budget', parseFloat(e.target.value))} className={`${inputClass} pl-10 font-bold text-slate-800 dark:text-slate-100`} placeholder="0.00" />
                                    </div>
                                    ...
                                    {formData.folderType === 'Sous-traitance' && formData.vatRate !== 0 && (
                                        <p className="text-[10px] text-orange-600 font-bold mt-1 flex items-center">
                                            <AlertTriangle size={10} className="mr-1" /> Attention: Un dossier sous-traitance est souvent en autoliquidation (0%).
                                        </p>
                                    )}
                                </div>
                                <div>
                                    <label className={labelClass}>Description / Notes</label>
                                    <textarea rows={6} value={formData.description} onChange={e => updateField('description', e.target.value)} className={`${inputClass} resize-none`} />
                                </div>

                                {/* --- TASKS MODULE (NEW) --- */}
                                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <h4 className="font-bold text-slate-700 dark:text-slate-200 dark:text-white text-sm flex items-center mb-3">
                                        <ListChecks size={16} className="mr-2" /> Prestations & Tâches (Visible sur Fiche Chantier)
                                    </h4>
                                    ...
                                    <form onSubmit={handleAddTask} className="flex mb-3">
                                        ...
                                        />
                                        <button type="submit" className="bg-slate-800 text-white px-3 py-2 rounded-r-lg text-sm font-bold hover:bg-emerald-600 transition-colors">
                                            <Plus size={16} />
                                        </button>
                                    </form>
                                    ...
                                    <div className="space-y-1 max-h-40 overflow-y-auto">
                                        {formData.tasks && formData.tasks.length > 0 ? (
                                            formData.tasks.map(task => (
                                                <div key={task.id} className="flex items-center group bg-white dark:bg-slate-900 dark:bg-slate-800/50 p-2 rounded border border-transparent hover:border-slate-200 transition-all">
                                                    <button
                                                    ...
                                                    <span className={`flex-1 text-sm ${task.isDone ? 'text-slate-700 dark:text-slate-200 dark:text-white line-through' : 'text-slate-700 dark:text-slate-200 dark:text-white'}`}>
                                                        {task.label}
                                                    </span>
                                                    <button
                                            ...
                                            <div className="text-xs text-slate-700 dark:text-slate-200 dark:text-white italic text-center py-2">Aucune prestation listée.</div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center justify-between">
                                        <h4 className="font-bold text-slate-700 dark:text-slate-200 dark:text-white text-sm">Levées de Réserves</h4>
                                        <button
                                        ...
                                        >
                                            <span className={`${formData.hasReserves ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white dark:bg-slate-900 transition-transform`} />
                                        </button>
                                    </div>
                                    {formData.hasReserves && (
                                        <div className="mt-3 bg-orange-50 dark:bg-orange-900/20 p-3 rounded-lg border border-orange-100 dark:border-orange-800 space-y-3">
                                            <div>
                                                ...
                                                <input type="date" value={formData.reservesDate || ''} onChange={e => updateField('reservesDate', e.target.value)} className={inputClass} />
                                            </div>
                                            <div>
                                                <label className={labelClass}>Date Réserves Levées (Résolues)</label>
                                                <div className="flex space-x-2">
                                                    <input type="date" value={formData.reservesResolvedDate || ''} onChange={e => updateField('reservesResolvedDate', e.target.value)} className={inputClass} />
                                                    {formData.reservesResolvedDate && <CheckSquare className="text-emerald-500 mt-2" />}
                                                </div>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className="col-span-1 md:col-span-2 mt-8 pt-8 border-t border-slate-200 dark:border-slate-800">
                                <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-lg p-4 flex justify-between items-center">
                                    <div>
                                        <h4 className="text-sm font-bold text-red-800 dark:text-red-300">Zone de Danger</h4>
                                        <p className="text-xs text-red-600 dark:text-red-400 mt-1">La suppression est définitive et irréversible.</p>
                                    </div>
                                    <button
                    ...
                    {activeTab === 'Sous-traitance' && (
                        <div className="space-y-6">
                            <div className="flex justify-between items-center">
                                <h3 className={sectionTitleClass}><ShoppingCart size={18} className="mr-2" /> Bons de Commande & Achats</h3>
                                <button
                                ...
                                <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 mb-6 animate-in fade-in">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            {!newBDC.subId && (
                                                <p className="text-[10px] text-slate-700 dark:text-slate-200 dark:text-white mt-1">Si le partenaire n'est pas dans la liste, ajoutez-le d'abord dans l'onglet Partenaires.</p>
                                            )}
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {formData.purchaseOrders?.map(po => (
                                    <div key={po.id} className="border border-slate-200 rounded-lg p-4 bg-white dark:bg-slate-900 dark:bg-slate-800/50">
                                        <div className="flex justify-between mb-2">
                                            <span className="font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">{po.number}</span>
                                            <span className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-700 dark:text-slate-200">{new Date(po.date).toLocaleDateString()}</span>
                                        </div>
                                        <p className="text-sm font-medium">{po.subcontractorName}</p>
                                        <p className="text-xs text-slate-700 dark:text-slate-200 dark:text-white mb-2">{po.description}</p>
                                        <div className="font-mono font-bold text-lg mb-4">{po.amountHT.toLocaleString()} € HT</div>
                                        <div className="flex space-x-2">
                                            <button onClick={() => printBDC(po)} className="flex items-center text-xs bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded font-bold text-slate-700 dark:text-slate-200" title="Imprimer (Simple)">
                                                <Printer size={14} className="mr-1" /> Imprimer
                                            </button>
                                            <button onClick={() => generatePurchaseOrderPDF(po, formData)} className="flex items-center text-xs bg-emerald-100 hover:bg-emerald-200 px-3 py-2 rounded font-bold text-emerald-700" title="Télécharger PDF Pro">
                                                <Download size={14} className="mr-1" /> PDF
                                            </button>
                                            <a href={generateWorkOrderEmailLink('email@exemple.com', po, formData)} className="flex items-center text-xs bg-blue-100 hover:bg-blue-200 px-3 py-2 rounded font-bold text-blue-700" title="Envoyer par Email">
                    ...
                    {activeTab === 'Contact' && (
                        <div className="space-y-6">
                            <h3 className={sectionTitleClass}><User size={18} className="mr-2" /> Coordonnées Client / Siège</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <div><label className={labelClass}>Nom Complet / Raison Sociale</label><input type="text" value={formData.client?.name || ''} onChange={e => setFormData(prev => ({ ...prev, client: { ...prev.client, name: e.target.value } }))} className={inputClass} /></div>
                                    <div><label className={labelClass}>Email</label><input type="email" value={formData.client?.email || ''} onChange={e => setFormData(prev => ({ ...prev, client: { ...prev.client, email: e.target.value } }))} className={inputClass} /></div>
                                    <div>
                                        <label className={labelClass}>Téléphone</label>
                                        <div className="flex space-x-2">
                                            <input type="tel" value={formData.client?.phone || ''} onChange={e => setFormData(prev => ({ ...prev, client: { ...prev.client, phone: e.target.value } }))} className={inputClass} />
                                ...
                                <div className="space-y-4">
                                        ...
                                        <div className="flex space-x-2">
                                            <div className="relative w-full">
                                                <AddressAutocomplete
                                                    ...
                                                    onChange={(val) => setFormData(prev => ({ ...prev, client: { ...prev.client, address: val } }))}
                                            ...
                                            <button
                                                onClick={() => copyToClipboard(`${formData.client?.address} ${formData.client?.zipCode} ${formData.client?.city}`)}
                                                className="bg-slate-100 hover:bg-slate-200 p-2.5 rounded-lg transition-colors flex items-center justify-center text-slate-700 dark:text-slate-200"
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className={labelClass}>Code Postal</label><input type="text" value={formData.client?.zipCode || ''} onChange={e => setFormData(prev => ({ ...prev, client: { ...prev.client, zipCode: e.target.value } }))} className={inputClass} /></div>
                                        <div><label className={labelClass}>Ville</label><input type="text" value={formData.client?.city || ''} onChange={e => setFormData(prev => ({ ...prev, client: { ...prev.client, city: e.target.value } }))} className={inputClass} /></div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {activeTab === 'Document' && (
                        <div className="space-y-6">
                            <h3 className={sectionTitleClass}><FileText size={18} className="mr-2" /> Documents</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                                <div className="p-4 rounded-xl border border-dashed border-sky-300 bg-sky-50 dark:bg-sky-900/10 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-sky-100 relative group transition-all">
                                    {isUploading ? <Loader2 className="animate-spin text-sky-500 mb-2" /> : <Upload size={24} className="text-sky-500 mb-2 group-hover:scale-110 transition-transform" />}
                                    <span className="text-sm font-bold text-sky-700">Ajouter DEVIS</span>
                                    <input type="file" accept="application/pdf,image/*" disabled={isUploading} className="absolute inset-0 opacity-0 cursor-pointer disabled:cursor-not-allowed" onChange={(e) => handleFileUpload(e, 'QUOTE')} />
                                </div>
                                <div className="p-4 rounded-xl border border-dashed border-emerald-300 bg-emerald-50 dark:bg-emerald-900/10 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-emerald-100 relative group transition-all">
                                    {isUploading ? <Loader2 className="animate-spin text-emerald-500 mb-2" /> : <Upload size={24} className="text-emerald-500 mb-2 group-hover:scale-110 transition-transform" />}
                                    <span className="text-sm font-bold text-emerald-700">Ajouter PV</span>
                                    <input type="file" accept="application/pdf,image/*" disabled={isUploading} className="absolute inset-0 opacity-0 cursor-pointer disabled:cursor-not-allowed" onChange={(e) => handleFileUpload(e, 'PV')} />
                                </div>
                                <div className="p-4 rounded-xl border border-dashed border-violet-300 bg-violet-50 dark:bg-violet-900/10 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-violet-100 relative group transition-all">
                                    {isUploading ? <Loader2 className="animate-spin text-violet-500 mb-2" /> : <Upload size={24} className="text-violet-500 mb-2 group-hover:scale-110 transition-transform" />}
                                    <span className="text-sm font-bold text-violet-700">Ajouter FACTURE</span>
                                    <input type="file" accept="application/pdf,image/*" disabled={isUploading} className="absolute inset-0 opacity-0 cursor-pointer disabled:cursor-not-allowed" onChange={(e) => handleFileUpload(e, 'INVOICE')} />
                                </div>
                                <div className="p-4 rounded-xl border border-dashed border-slate-300 bg-slate-50 dark:bg-slate-900/10 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-slate-100 relative group transition-all">
                                    {isUploading ? <Loader2 className="animate-spin text-slate-700 dark:text-slate-200 dark:text-white mb-2" /> : <Upload size={24} className="text-slate-700 dark:text-slate-200 dark:text-white mb-2 group-hover:scale-110 transition-transform" />}
                                    <span className="text-sm font-bold text-slate-700 dark:text-slate-200 dark:text-white">Ajouter AUTRE</span>
                                    <input type="file" accept="application/pdf,image/*" disabled={isUploading} className="absolute inset-0 opacity-0 cursor-pointer disabled:cursor-not-allowed" onChange={(e) => handleFileUpload(e, 'AUTRE')} />
                                </div>
                            </div>

                            <table className="w-full text-sm text-left">
                                <thead className="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 dark:text-white font-medium">
                                    <tr><th className="px-4 py-3">Type</th><th className="px-4 py-3">Nom</th><th className="px-4 py-3">Date</th><th className="px-4 py-3 text-right">Actions</th></tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                    {(!formData.documents || formData.documents.length === 0) ? (
                                        <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-700 dark:text-slate-200 dark:text-white">Aucun document.</td></tr>
                                    ) : (
                                        formData.documents.map(doc => (
                                            <tr key={doc.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                <td className="px-4 py-3">
                                                    <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${doc.type === 'QUOTE' ? 'bg-sky-100 text-sky-700' :
                                                        doc.type === 'INVOICE' ? 'bg-violet-100 text-violet-700' :
                                                            doc.type === 'PV' ? 'bg-emerald-100 text-emerald-700' :
                                                                'bg-slate-100 text-slate-700 dark:text-slate-200'
                                                        }`}>
                                                        {doc.type === 'QUOTE' ? 'DEVIS' : doc.type === 'INVOICE' ? 'FACTURE' : doc.type}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100 dark:text-white">{doc.name}</td>
                                                <td className="px-4 py-3 text-slate-700 dark:text-slate-200 dark:text-white">{new Date(doc.date).toLocaleDateString()}</td>
                                                <td className="px-4 py-3 text-right">
                                                    <button onClick={() => setPreviewDoc(doc)} className="text-emerald-600 mr-2 hover:bg-emerald-50 p-1.5 rounded transition-colors" title="Voir"><Eye size={16} /></button>
                                                    <button onClick={() => setFormData(prev => ({ ...prev, documents: prev.documents?.filter(d => d.id !== doc.id) }))} className="text-red-500 hover:bg-red-50 p-1.5 rounded transition-colors" title="Supprimer"><Trash2 size={16} /></button>
                                                </td>
                                            </tr>
                                        ))
                                    )}
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                                    {['AVANT', 'PENDANT', 'APRES'].map((tab) => (
                                        <button key={tab} onClick={() => setActivePhotoTab(tab as any)} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${activePhotoTab === tab ? 'bg-white dark:bg-slate-900 dark:bg-slate-600 shadow text-slate-900 dark:text-white dark:text-white dark:text-white' : 'text-slate-700 dark:text-slate-200 dark:text-white'}`}>{tab}</button>
                                    ))}
                                    <button onClick={() => setActivePhotoTab('COMPARATIF')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${activePhotoTab === 'COMPARATIF' ? 'bg-white dark:bg-slate-900 dark:bg-slate-600 shadow text-indigo-600' : 'text-indigo-500'}`}>
                                        COMPARATIF
                                    </button>
                                </div>
                                {activePhotoTab !== 'COMPARATIF' && (
                                    <div className="relative">
                                        <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg flex items-center text-sm font-medium disabled:opacity-50 shadow-sm">
                                            {isUploading ? <Loader2 size={16} className="mr-2 animate-spin" /> : <Upload size={16} className="mr-2" />} Ajouter
                                        </button>
                            ...
                            {activePhotoTab === 'COMPARATIF' ? (
                                <div className="grid grid-cols-2 gap-8 h-[400px]">
                                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800 p-4 flex flex-col">
                                        <h4 className="font-bold text-center text-slate-700 dark:text-slate-200 dark:text-white mb-2">AVANT</h4>
                                        <div className="flex-1 overflow-y-auto space-y-2">
                                            {formData.photos?.filter(p => p.type === 'AVANT').map(p => (
                                                <img key={p.id} src={p.url} loading="lazy" decoding="async" className="w-full rounded-lg shadow-sm" alt="Avant" />
                                            ))}
                                            {!formData.photos?.some(p => p.type === 'AVANT') && <p className="text-center text-xs text-slate-700 dark:text-slate-200 dark:text-white mt-10">Aucune photo "Avant"</p>}
                                        </div>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800 p-4 flex flex-col">
                                        <h4 className="font-bold text-center text-slate-700 dark:text-slate-200 dark:text-white mb-2">APRÈS</h4>
                                        <div className="flex-1 overflow-y-auto space-y-2">
                                            {formData.photos?.filter(p => p.type === 'APRES').map(p => (
                                                <img key={p.id} src={p.url} loading="lazy" decoding="async" className="w-full rounded-lg shadow-sm" alt="Après" />
                                            ))}
                                            {!formData.photos?.some(p => p.type === 'APRES') && <p className="text-center text-xs text-slate-700 dark:text-slate-200 dark:text-white mt-10">Aucune photo "Après"</p>}
                                        </div>
                                    </div>
                                </div>
                            ) : (
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {formData.photos?.filter(p => p.type === activePhotoTab).map(photo => (
                                        <div key={photo.id} className="relative aspect-square bg-slate-100 rounded-xl overflow-hidden group shadow-sm">
                                            <img src={photo.url} loading="lazy" decoding="async" alt="Chantier" className="w-full h-full object-cover" />
                                            <button onClick={() => deletePhoto(photo.id)} className="absolute top-2 right-2 p-2 bg-white dark:bg-slate-900/90 rounded-full text-red-500 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm hover:scale-110">
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    ))}
                    ...
                    {showValidationModal && (
                        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 print:hidden">
                            <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-2xl p-6">
                                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white mb-4">Dates des travaux</h3>
                                <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white mb-4">Le devis est validé. Veuillez confirmer les dates prévues.</p>

                                <div className="space-y-4">
                                    <div><label className={labelClass}>Date Début</label><input type="date" value={validationDates.start} onChange={e => setValidationDates({ ...validationDates, start: e.target.value })} className={inputClass} /></div>
                                    <div><label className={labelClass}>Date Fin</label><input type="date" value={validationDates.end} onChange={e => setValidationDates({ ...validationDates, end: e.target.value })} className={inputClass} /></div>
                                </div>

                                <div className="flex justify-end space-x-2 mt-6">
                                    <button onClick={() => setShowValidationModal(false)} className="px-4 py-2 text-slate-700 dark:text-slate-200 hover:bg-slate-100 rounded-lg text-sm">Annuler</button>
                                    <button onClick={confirmValidation} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-bold">Confirmer</button>
                                </div>
                            </div>
                        </div>
                    )}
