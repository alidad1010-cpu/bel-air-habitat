...
const ClientDetailModal: React.FC<ClientDetailModalProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'INFO' | 'DOCS' | 'CONTACTS'>('INFO');
                                        ...
                                        {isEditing ? (
                                            ...
                                            formData.tenant ? (
                                                <div className="text-sm">
                                                    <p className="font-bold text-slate-800 dark:text-slate-100">{formData.tenant.name}</p>
                                                    {formData.tenant.phone && <div className="flex items-center text-slate-600 dark:text-slate-300 mt-1"><Phone size={14} className="mr-2" /> <a href={`tel:${formData.tenant.phone}`} className="hover:text-emerald-600">{formData.tenant.phone}</a></div>}
                                                    {formData.tenant.email && <div className="flex items-center text-slate-600 dark:text-slate-300 mt-1"><Mail size={14} className="mr-2" /> <a href={`mailto:${formData.tenant.email}`} className="hover:text-emerald-600">{formData.tenant.email}</a></div>}
                                                </div>
                                            ) : (
                                                <p className="text-sm text-slate-400 italic">Aucun locataire renseigné</p>
                                            )
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            {isEditing && (
                                <button
                                    onClick={() => setFormData({
                                        ...formData,
                                        contacts: [...(formData.contacts || []), { id: Date.now().toString(), name: '', role: '', email: '', phone: '' }]
                                    })}
                                    className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center shadow-lg shadow-emerald-200 dark:shadow-none"
                                >
                        ...
                        <div className="space-y-4 overflow-y-auto pr-2">
                            {(!formData.contacts || formData.contacts.length === 0) ? (
                                ...
                                formData.contacts.map((contact, idx) => (
                                    <div key={contact.id} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-emerald-300 transition-colors relative group">
                                        {isEditing && (
                                            <button
                                                onClick={() => {
                                                    const newContacts = formData.contacts?.filter(c => c.id !== contact.id);
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="space-y-3">
                                                <div>
                                                    <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Nom</label>
                                                    {isEditing ? (
                                                        <input
                                                            type="text"
                                                            value={contact.name}
                                                ...
                                                <div>
                                                    <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Rôle / Poste</label>
                                                    {isEditing ? (
                                                        <input
                                                            type="text"
                                                            value={contact.role || ''}
                                                            ...
                                                            className="w-full p-2 text-sm border rounded dark:bg-slate-800 dark:border-slate-600 dark:text-white"
                                                            placeholder="Ex: Comptable, Gardien..."
                                                        />
                                                    ) : (
                                            ...
                                            <div className="space-y-3">
                                                <div>
                                                    <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Email</label>
                                                    {isEditing ? (
                                                        <input
                                                            type="email"
                                                            value={contact.email || ''}
                                                        ...
                                                        <a href={`mailto:${contact.email}`} className="text-sm text-emerald-600 hover:underline flex items-center">
                                                            <Mail size={14} className="mr-2" /> {contact.email || '-'}
                                                        </a>
                                                    )}
                                                </div>
                                                <div>
                                                    <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Téléphone</label>
                                                    {isEditing ? (
                                                        <input
                                                            type="tel"
                                                            value={contact.phone || ''}
                                                        ...
                                                        <a href={`tel:${contact.phone}`} className="text-sm text-slate-700 dark:text-slate-300 hover:text-emerald-600 flex items-center">
