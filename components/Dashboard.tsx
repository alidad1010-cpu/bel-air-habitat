...
const StatCard: React.FC<StatCardProps> = ({
  ...
  // Enhanced themes with soft, clear colors for the entire card
  const themes: Record<
    string,
    {
      cardBg: string;
  ...
  > = {
    blue: {
      ...
      countColor: 'text-blue-700 dark:text-blue-100',
    },
    emerald: {
    ...
    rose: {
      ...
      countColor: 'text-rose-700 dark:text-rose-100',
    },
    amber: {
      ...
      countColor: 'text-amber-700 dark:text-amber-100',
    },
    violet: {
      ...
      countColor: 'text-violet-700 dark:text-violet-100',
    },
    sky: {
      ...
      countColor: 'text-sky-700 dark:text-sky-100',
    },
    slate: {
      ...
      countColor: 'text-slate-700 dark:text-white',
    },
  };
  ...
  return (
    <div
      onClick={onClick}
      className={`glass-card relative rounded-2xl p-6 cursor-pointer group flex items-start justify-between min-h-[140px] transition-all hover:scale-[1.02] border ${theme.cardBg} ${theme.cardBorder}`}
    >
      <div className="flex flex-col justify-between h-full">
        <div className={`text-5xl font-black tracking-tight mb-2 ${theme.countColor}`}>{count}</div>
        <div
          className={`font-bold text-sm leading-tight uppercase tracking-wide ${theme.textColor}`}
        >
          {title}
        </div>
      </div>
      ...
      <div
        className={`p-3 rounded-xl flex items-center justify-center shadow-sm ${theme.iconBg} ${theme.iconColor}`}
      >
        <Icon size={24} />
      </div>
    </div>
        ...
        {notes.length === 0 ? (
          ...
          notes.map((note) => {
            ...
            return (
              <div
                key={note.id}
                className="bg-slate-50/50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800 hover:bg-white dark:bg-slate-900 dark:hover:bg-slate-800 transition-all group relative backdrop-blur-sm"
              >
                <div className="flex justify-between items-start mb-1">
                  <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-200 uppercase tracking-wide">
                    {note.authorName}
                  </span>
                ...
                <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white leading-relaxed">
                  {note.content}
                </p>
                {isAuthor && onDelete && (
                  <button
...
const FinancialOverview = ({ projects }: { projects: Project[] }) => {
  const stats = useMemo(() => {
    ...
    projects.forEach((p) => {
      ...
      if (
        [ProjectStatus.VALIDATED, ProjectStatus.IN_PROGRESS, ProjectStatus.COMPLETED].includes(
          p.status
        )
      )
        secured += amount;
    ...
    <div className="glass-card rounded-2xl p-6 h-full flex flex-col justify-between">
      <div className="flex items-center mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-700 dark:text-slate-200 dark:text-white font-medium">
            Flux de trésorerie (HT)
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="space-y-1">
          <div className="text-[10px] font-bold uppercase text-emerald-600 dark:text-emerald-200 tracking-wider">
            Sécurisé
          </div>
          <div className="text-xl font-black text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
            {format(stats.secured)}
          </div>
        ...
        <div className="space-y-1">
          <div className="text-[10px] font-bold uppercase text-slate-700 dark:text-slate-200 dark:text-white tracking-wider">
            En Clôture
          </div>
          <div className="text-xl font-black text-slate-800 dark:text-slate-100 dark:text-white">
            {format(stats.waiting)}
          </div>
        ...
        <div className="space-y-1">
          <div className="text-[10px] font-bold uppercase text-amber-500 dark:text-amber-200 tracking-wider">
            Potentiel
          </div>
          <div className="text-xl font-black text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
            {format(stats.potential)}
          </div>
      ...
      {stats.total > 0 && (
        <div className="relative pt-2 mt-auto">
          <div className="flex justify-between text-xs font-bold mb-2">
            <span className="text-slate-700 dark:text-slate-200 dark:text-white">
          ...
          <div className="h-4 w-full bg-slate-100 dark:bg-slate-900/50 rounded-lg overflow-hidden flex ring-1 ring-slate-200 dark:ring-slate-700">
            ...
            <div
              ...
              className="bg-slate-500 h-full border-r border-white/20"
              title="En Clôture"
            ></div>
            <div
              style={{ width: `${(stats.potential / stats.total) * 100}%` }}
  ...
  useEffect(() => {
    ...
    if (validProjects.length > 0) {
      ...
      validProjects.forEach((p) => {
        ...
        marker.bindPopup(
          `<div style="font-family: 'Outfit', sans-serif; color: #1e293b;"><strong>${p.title}</strong><br/>${p.client.name}<br/>${p.status}</div>`
        );
      });
...
const GlobalStats = ({ projects }: { projects: Project[] }) => {
  const stats = useMemo(() => {
    ...
    const lost = projects.filter((p) =>
      [ProjectStatus.LOST, ProjectStatus.REFUSED, ProjectStatus.CANCELLED].includes(p.status)
    ).length;
  ...
  return (
    <div className="mt-12 mb-8 animate-fade-in-up">
      <div className="flex items-center mb-6 px-1">
        ...
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card rounded-2xl p-6 flex items-center justify-between border-b-4 border-indigo-500">
          <div>
            <div className="text-indigo-600 dark:text-white text-xs font-black uppercase tracking-wider mb-2">
              Total Dossiers
            </div>
            <div className="text-5xl font-black text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
              {stats.total}
            </div>
          </div>
          <div className="bg-indigo-50 dark:bg-indigo-900/30 p-4 rounded-2xl text-indigo-500">
            {' '}
            <List size={32} />{' '}
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 flex items-center justify-between border-b-4 border-emerald-500">
          <div>
            <div className="text-emerald-600 dark:text-white text-xs font-black uppercase tracking-wider mb-2">
              Taux de Succès
            </div>
            <div className="text-5xl font-black text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
          ...
          <div className="bg-emerald-50 dark:bg-emerald-900/30 p-4 rounded-2xl text-emerald-500">
            {' '}
            <Trophy size={32} />{' '}
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 flex items-center justify-between border-b-4 border-rose-500">
          <div>
            <div className="text-rose-600 dark:text-white text-xs font-black uppercase tracking-wider mb-2">
              Perdus
            </div>
            <div className="text-5xl font-black text-slate-800 dark:text-slate-100 dark:text-white dark:text-white">
              {stats.lost}
          ...
          <div className="bg-rose-50 dark:bg-rose-900/30 p-4 rounded-2xl text-rose-500">
            {' '}
            <Archive size={32} />{' '}
          </div>
        </div>
    ...
    return {
      ...
      late: projects.filter((p) => {
        ...
        return (
          ...
          p.status !== ProjectStatus.REFUSED &&
        ...
        <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-slate-600 dark:text-slate-400 font-medium">
                {currentUser?.fullName || 'anwishmukhtar'} •{' '}
          ...
          <div className="flex flex-col items-end space-y-3">
            ...
            <div className="flex bg-slate-900 border border-slate-700 p-1 rounded-xl items-center">
              ...
              >
                <CheckSquare size={18} className="mr-2" /> Tâches
              </button>
              <div className="w-px bg-slate-700 mx-1"></div>
              <button
                onClick={() => setViewMode('LIST')}
                className={`p-2 rounded-lg transition-all ${viewMode === 'LIST' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                <List size={20} />
              </button>
              <button
                onClick={() => setViewMode('MAP')}
                className={`p-2 rounded-lg transition-all ${viewMode === 'MAP' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                <Map size={20} />
              </button>
            </div>
        ...
        {viewMode === 'LIST' ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2">
              ...
              >
                ...
                <h3 className="text-3xl font-bold text-slate-800 dark:text-slate-100 dark:text-white mb-2 group-hover:text-indigo-600 dark:group-hover:text-white transition-colors">
                  Mon Planning
                </h3>
            ...
            <div className="md:col-span-1">
              <div className="glass-card rounded-3xl p-8 h-64 flex flex-col justify-center items-center text-center">
                <div className="text-sm font-bold text-slate-700 dark:text-slate-200 dark:text-white uppercase tracking-widest mb-4">
                  Prochain RDV
                </div>
                <div className="text-xl font-bold text-slate-800 dark:text-slate-100 dark:text-white dark:text-white mb-2">
                  Aucun RDV imminent
                </div>
                <p className="text-slate-700 dark:text-slate-200 dark:text-white text-sm">
                  Profitez-en pour traiter vos dossiers !
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card rounded-3xl p-4 border border-indigo-100 dark:border-slate-800">
            <div className="flex justify-between items-center mb-4 px-2">
              <h3 className="font-bold text-lg flex items-center text-indigo-900 dark:text-white dark:text-white">
                <MapPin className="mr-2 text-indigo-500" /> Carte des Chantiers
              </h3>
              <span className="text-xs font-bold text-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1 rounded-full">
                Vue Satellite
              </span>
            </div>
            <div className="rounded-2xl overflow-hidden shadow-inner border border-slate-200 dark:border-slate-800">
              <LeafletMap
                projects={projects.filter(
                  (p) =>
                    p.status === ProjectStatus.VALIDATED || p.status === ProjectStatus.IN_PROGRESS
                )}
              />
            </div>
          </div>
