...
import { Search, Plus, Trash2, User, Upload, Printer, Globe, Save, X, ExternalLink, Briefcase, DollarSign, Camera, FileText, Download, Loader2, Clock, Calendar, CheckSquare, FileSpreadsheet, ChevronLeft, ChevronRight, Ban, RotateCw } from 'lucide-react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { Employee, EmployeeDocument, Expense, ExpenseCategory, ExpenseType, AttendanceRecord, AttendanceStatus } from '../types';
...
const EmployeesPage: React.FC<EmployeesPageProps> = ({
    ...
    attendances = [], onUpdateAttendance, onBulkUpdateAttendance, onBulkDeleteAttendance
}) => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: EmployeeDocument['type']) => {
        ...
        try {
            ...
            try {
                ...
                if (fileToUpload.size > 4.5 * 1024 * 1024) throw new Error("Fichier trop lourd pour le mode hors-ligne.");
    ...
    const handleBulkStatusUpdate = (status: AttendanceStatus) => {
        if (!confirm(`Marquer tous les salariés comme "${status}" pour ce mois ? (Attention, cela écrasera les données existantes)`)) return;
    ...
    const handleResetMonth = () => {
        if (!confirm("Voulez-vous vraiment réinitialiser (effacer) toutes les présences pour ce mois ? Cette action est irréversible.")) return;
    ...
    const exportPayroll = () => {
        ...
        doc.setFontSize(12);
        doc.text(`Rapport Mensuel de Paie - ${currentDate.toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}`, 14, 30);
    ...
    const inputClass = "w-full p-2.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white placeholder-slate-400 text-sm";
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 print:hidden">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-700 dark:text-white text-sm">Ressources Humaines & Comptabilité</p>
                    </div>
                </div>
                                ...
                                {filteredEmployees.length === 0 ? (
                                    ...
                                    filteredEmployees.map(emp => (
                                        <tr key={emp.id} onClick={() => handleOpenModal(emp)} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer transition-colors group">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-bold ${emp.isForeigner ? 'bg-orange-100 text-orange-700 border border-orange-200' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-white'}`}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
                        ...
                        <div className="flex items-center space-x-2">
                            <button onClick={handleResetMonth} className="text-xs font-bold bg-slate-100 text-slate-700 px-3 py-2 rounded-lg hover:bg-slate-200 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white flex items-center">
                                <RotateCw size={14} className="mr-1" /> Réinitialiser
                            </button>
                            <button onClick={() => handleBulkStatusUpdate(AttendanceStatus.PRESENT)} className="text-xs font-bold bg-emerald-100 text-emerald-700 px-3 py-2 rounded-lg hover:bg-emerald-200 flex items-center">
                    ...
                    {selectedCells.size > 0 && (
                        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full shadow-2xl flex items-center space-x-6 z-50 animate-in slide-in-from-bottom-4">
                            ...
                            <button onClick={() => setIsTimeModalOpen(true)} className="flex items-center hover:text-emerald-400 font-bold text-sm">
                                <Clock size={16} className="mr-2" /> Appliquer Horaire
                            </button>
                            <button onClick={handleBulkClearSelection} className="flex items-center hover:text-red-400 font-bold text-sm">
                                <Trash2 size={16} className="mr-2" /> Effacer
                            </button>
                            <button onClick={() => setSelectedCells(new Set())} className="ml-4 p-1 hover:bg-slate-800 rounded-full">
                                <X size={16} />
                            </button>
                        </div>
                    )}
                    ...
                    {isTimeModalOpen && (
                        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-sm shadow-2xl space-y-4 animate-in zoom-in-95">
                                <h3 className="text-lg font-bold text-slate-800 dark:text-white">Définir l'Horaire</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Début</label>
                                        <input type="time" value={timeFormData.startTime} onChange={e => setTimeFormData({ ...timeFormData, startTime: e.target.value })} className="w-full p-2 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Fin</label>
                                        <input type="time" value={timeFormData.endTime} onChange={e => setTimeFormData({ ...timeFormData, endTime: e.target.value })} className="w-full p-2 bg-slate-100 dark:bg-slate-800 rounded-lg" />
                                    </div>
                                </div>
                                <div className="flex justify-end space-x-2 pt-2">
                                    <button onClick={() => setIsTimeModalOpen(false)} className="px-4 py-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg font-bold text-sm">Annuler</button>
                                    <button onClick={handleApplySchedule} className="px-4 py-2 bg-emerald-600 text-white rounded-lg font-bold text-sm hover:bg-emerald-700">Appliquer</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {/* Time Grid */}
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-x-auto select-none" onMouseLeave={handleMouseUp}>
                        <table className="w-full text-xs text-center border-collapse">
                            <thead>
                                <tr>
                                    <th className="p-4 text-left min-w-[150px] sticky left-0 bg-white dark:bg-slate-900 text-slate-800 dark:text-white z-10 border-b border-slate-200 dark:border-slate-800">Salarié</th>
                                    {daysOfMonth.map(day => (
                                        <th key={day.toISOString()} className={`p-2 min-w-[40px] border-l border-slate-100 dark:border-slate-800 ${day.getDay() === 0 || day.getDay() === 6 ? 'bg-slate-50 dark:bg-slate-800/50 text-slate-400' : 'text-slate-800 dark:text-white'}`}>
                                            <div className="font-bold">{day.getDate()}</div>
                                            <div className="text-[10px] uppercase">{day.toLocaleDateString('fr-FR', { weekday: 'short' })}</div>
                                        </th>
                                    ))}
                                    <th className="p-4 min-w-[80px] font-bold text-slate-800 dark:text-white border-l border-b border-slate-200 dark:border-slate-800">Total H</th>
                                </tr>
                            </thead>
                            <tbody>
                                {employees.map(emp => {
                                    let totalHours = 0;
                                    return (
                                        <tr key={emp.id} className="border-t border-slate-100 dark:border-slate-800 group hover:bg-slate-50 dark:hover:bg-slate-800/30">
                                                ...
                                                return (
                                                    <td
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            if (!isDragging && selectedCells.size <= 1) handleCellClick(emp.id, dateStr);
                                                        }}
                    ...
                    <div className="flex gap-4 text-xs text-slate-500 justify-center">
                        <div className="flex items-center"><span className="w-3 h-3 bg-emerald-100 mr-1 rounded"></span> Présent</div>
                        <div className="flex items-center"><span className="w-3 h-3 bg-blue-100 mr-1 rounded"></span> Congé Payé</div>
                        <div className="flex items-center"><span className="w-3 h-3 bg-red-100 mr-1 rounded"></span> Maladie</div>
                        <div className="flex items-center"><span className="w-3 h-3 bg-orange-100 mr-1 rounded"></span> Injustifié</div>
                    </div>
                </div>
            )}
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in-95">
                        ...
                        {selectedEmployee && (
                            <div className="px-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800 flex space-x-4">
                                ...
                                <button onClick={() => setActiveTab('EXPENSES')} className={`py-3 text-sm font-bold border-b-2 ${activeTab === 'EXPENSES' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-slate-500'}`}>Ses Notes de Frais</button>
                            </div>
                        )}
                                    ...
                                    <div className="col-span-2 md:col-span-1 space-y-6">
                                        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div><label className={labelClass}>Nationalité</label><input type="text" value={formData.nationality} onChange={e => setFormData({ ...formData, nationality: e.target.value })} className={inputClass} placeholder="Ex: Française" /></div>
                            ...
                            {activeTab === 'EXPENSES' && (
                                <div className="space-y-6">
                                    ...
                                    <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                        <div>
                                            <h4 className="font-bold text-slate-800 dark:text-white">Historique des Frais</h4>
                                            <p className="text-xs text-slate-500">Pour {selectedEmployee?.firstName} {selectedEmployee?.lastName}</p>
                                        </div>
                                        <div className="flex space-x-2">
                                            <button
                                                ...
                                                {employeeExpenses.length === 0 ? (
                                                    ...
                                                    employeeExpenses.map(exp => (
                                                        <tr key={exp.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                                            <td className="px-6 py-4 text-slate-700 dark:text-white">{new Date(exp.date).toLocaleDateString('fr-FR')}</td>
                                                            <td className="px-6 py-4 font-bold text-slate-800 dark:text-white">{exp.merchant}</td>
                                                            <td className="px-6 py-4">
                                                            ...
                                                            <td className="px-6 py-4 text-right font-bold text-emerald-600">{exp.amount.toFixed(2)} €</td>
                                                            <td className="px-6 py-4 text-right text-slate-500 dark:text-slate-400">{(exp.vat || 0).toFixed(2)} €</td>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 print:hidden">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in-95">
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 shrink-0">
                            ...
                            <button onClick={() => setIsExpenseModalOpen(false)} className="text-slate-700 dark:text-white p-2">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 bg-slate-50 dark:bg-slate-900/50">
                            ...
                            {expenseMode === 'SELECT' && (
                                <div className="space-y-4">
                                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 text-center">Comment souhaitez-vous saisir cette dépense ?</p>
                                    ...
                                    >
                                        ...
                                        <div className="text-left">
                                            <div className="font-bold text-slate-800 dark:text-white">Saisie Manuelle</div>
                                    ...
                                    <div className="relative w-full">
                                        ...
                                        <div className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all pointer-events-none group">
                                            ...
                                            <div className="text-left">
                                                <div className="font-bold text-slate-800 dark:text-white">Scan Photo (IA)</div>
                                    ...
                                    <div className="relative w-full">
                                        ...
                                        <div className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all pointer-events-none group">
                                            ...
                                            <div className="text-left">
                                                <div className="font-bold text-slate-800 dark:text-white">Import Document (IA)</div>
                            ...
                            {expenseMode === 'FORM' && (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className={labelClass}>Date</label><input type="date" value={expenseFormData.date} onChange={e => setExpenseFormData({ ...expenseFormData, date: e.target.value })} className={inputClass} /></div>
                                        <div><label className={labelClass}>Montant TTC</label><input type="number" step="0.01" value={expenseFormData.amount} onChange={e => setExpenseFormData({ ...expenseFormData, amount: parseFloat(e.target.value) })} className={inputClass} /></div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className={labelClass}>TVA (Est.)</label><input type="number" step="0.01" value={expenseFormData.vat} onChange={e => setExpenseFormData({ ...expenseFormData, vat: parseFloat(e.target.value) })} className={inputClass} /></div>
                                        <div><label className={labelClass}>Fournisseur</label><input type="text" value={expenseFormData.merchant} onChange={e => setExpenseFormData({ ...expenseFormData, merchant: e.target.value })} className={inputClass} placeholder="Ex: Total Energies" /></div>
                                    </div>

                                    <div>
                                        <label className={labelClass}>Catégorie</label>
                                        <select value={expenseFormData.category} onChange={e => setExpenseFormData({ ...expenseFormData, category: e.target.value as ExpenseCategory })} className={inputClass}>
                                    ...
                                    <div>
                                        ...
                                        <textarea value={expenseFormData.notes} onChange={e => setExpenseFormData({ ...expenseFormData, notes: e.target.value })} className={`${inputClass} h-20 resize-none`} placeholder="Description de la dépense..." />
                                    </div>

                                    <button onClick={handleSaveExpense} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg transition-transform active:scale-95 flex justify-center items-center">
                                        <Save size={18} className="mr-2" /> Valider la Note
                                    </button>
                                    <button onClick={() => setExpenseMode('SELECT')} className="w-full py-2 text-slate-500 text-sm hover:underline">
                                        Retour
                                    </button>
                                </div>
                            )}
            ...
            {selectedCell && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 w-full max-w-sm animate-in fade-in zoom-in-95">
                        ...
                        <div className="space-y-3">
                            <button onClick={() => updateAttendanceStatus(AttendanceStatus.PRESENT)} className="w-full p-3 rounded-lg bg-emerald-100 text-emerald-700 font-bold hover:bg-emerald-200 flex items-center justify-center">Présent (Standard 7h)</button>
                            <button onClick={() => updateAttendanceStatus(AttendanceStatus.PAID_LEAVE)} className="w-full p-3 rounded-lg bg-blue-100 text-blue-700 font-bold hover:bg-blue-200 flex items-center justify-center">Congé Payé</button>
                            <button onClick={() => updateAttendanceStatus(AttendanceStatus.SICK_LEAVE)} className="w-full p-3 rounded-lg bg-red-100 text-red-700 font-bold hover:bg-red-200 flex items-center justify-center">Maladie</button>
                            <button onClick={() => updateAttendanceStatus(AttendanceStatus.UNJUSTIFIED)} className="w-full p-3 rounded-lg bg-orange-100 text-orange-700 font-bold hover:bg-orange-200 flex items-center justify-center">Absence Injustifiée</button>
                            <div className="border-t border-slate-100 my-2"></div>
                            <button onClick={() => setSelectedCell(null)} className="w-full p-2 text-slate-500 hover:bg-slate-100 rounded-lg">Annuler</button>
                        </div>
                    </div>
                </div>
            )}
